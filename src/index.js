...
const wahaFetch = (env, path, options = {}) => {
...
app.get('/api/wa-qr', async (c) => {
    try {
        ...
        if (!response.ok)
            return c.json({ error: 'QR tidak tersedia. Jalankan wa-login dahulu.' }, 404);
...
app.post('/api/tasks', async (c) => {
    ...
    const result = await c.env.DB.prepare('INSERT INTO tasks (name, title, description, deadline, phone_number) VALUES (?, ?, ?, ?, ?)').bind(name, title, description, deadline, phone_number).run();
...
// --- LOGIC BACKGROUND ---
async function syncSessionStatus(env) {
    try {
        const response = await wahaFetch(env, `/api/sessions/${SESSION_NAME}`);
        if (response.ok) {
            const data = await response.json();
            await env.DB.prepare('INSERT INTO wa_sessions (session_id, status) VALUES (?, ?) ON CONFLICT(session_id) DO UPDATE SET status = EXCLUDED.status').bind(SESSION_NAME, data.status).run();
        }
    }
    catch (e) {
...
async function sendReminder(env, task, type) {
    ...
    try {
        ...
        console.error(`Gagal mengirim reminder ${type}:`, e);
    }
}
